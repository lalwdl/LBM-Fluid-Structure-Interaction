#include "multi-particle.h"

//cd F:\2multi_particle
//icl/fast -o p multi-particle.cpp solidrate.cpp
...
void init()                           //初始化
{
    ...
    for (i = 0; i < 16; i++)
        for (j = 0; j < 8; j++) {
			x[j * 16 + i][1] = (double (NY)-15-7*len0) + j * len0;
			x[j * 16 + i][0] = (double (NX)-15*len0)/2 + i * len0;
        }
    dx = 1;
    dy = 1;
    Lx = dx * double(NX);
...
int main()
{
    ...
    for ( n = 0;; n++)
    {
        ...
        sum = 0;
        sum0 = 0;
        sum0 = Ff[0][0] + Fp_sum[0][0] + Fw[0][0];
		sum  = Ff[0][1] + Fp_sum[0][1] + mm0;
		if(n%20==0){
			out << setw(6) << n << "  " << setprecision(6)
				<< x[0][0] << "  " << Ff[0][0] <<"  "<< Fp_sum[0][0] << "  " << Fw[0][0] <<"  "<<sum0<<"  "<<velocity_central[0][0]<<"  "
				<< x[0][1] << "  " << Ff[0][1] <<"  "<< Fp_sum[0][1] << "  " << mm0      <<"  "<<sum <<"  "<<velocity_central[0][1]<<"  "
				<<angular[0]<<"  " <<angle[0]<<endl;
		}
        if (n % 100 == 0)
        {
            Error();
